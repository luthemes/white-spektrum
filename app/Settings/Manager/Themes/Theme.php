...
namespace WhiteSpektrum\Settings\Manager\Themes;
			...
			<div class="theme-id-container">
				...
				<h2 class="theme-name" id="<?php echo esc_attr( sprintf( '%s-name', $this->name() ) ) ?>">
					<?php if ( $this->active() ) : ?>
						<?php printf(
							'<span>%s</span> %s',
							esc_html__( 'Active:', 'white-spektrum' ),
							esc_html( $this->label() )
						) ?>
					<?php else : ?>
		...
		if ( $this->installed() ) {
			...
			if ( $this->active() && current_user_can( 'customize' ) ) {
				...
				$actions[] = sprintf(
					...
					esc_html__( 'Customize', 'white-spektrum' )
				);
			}
			...
			if ( ! $this->active() && current_user_can( 'switch_themes' ) ) {
				...
				$actions[] = sprintf(
					...
					esc_attr( sprintf( __( 'Activate %s', 'white-spektrum' ), $this->label() ) ),
					esc_html__( 'Activate', 'white-spektrum' )
				);
			}
			...
			if ( ! $this->active() && current_user_can( 'customize' ) ) {
				...
				$actions[] = sprintf(
					'<a class="button button-primary load-customize hide-if-no-customize" href="%s">%s</a>',
					esc_url( $this->customizeUrl() ),
					esc_html__( 'Live Preview', 'white-spektrum' )
				);
			}
			...
			$actions[] = sprintf(
				...
				esc_html__( 'Download', 'white-spektrum' )
			);
		}
